...
def house_price():
    ...
    LAND_AREA = st.sidebar.slider('land area', 60, 3000, 60)
    FLOOR_AREA = st.sidebar.slider('floor area', 60, 900, 60)
    ...
    GARAGE = st.sidebar.slider('garage', 1, 5, 1)
    NEAREST_SCH_RANK = st.sidebar.slider('school rank', 1, 139, 1)
    CBD_DIST = st.sidebar.slider('central business district distance', 680, 6000, 680)
...
st.subheader('$' + str(np.round(price[0], 0)))
